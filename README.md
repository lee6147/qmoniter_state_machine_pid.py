...
class TurtleMonitor(Node):
    def __init__(self):
        ...
        # self.create_subscription(Pose, 'target_pose', self.goal_pose_callback, 10)
        ...
        # self.target_publisher = self.create_publisher(PoseStamped, 'target_pose', 10)
#
...
class MainWindow(QWidget):
    ...
    def on_mouse_release(self, event):
        if event.button == 1 and self.drag_start and self.drag_current:
            ...
            self.ros_thread.node.get_logger().info(
                f"Published new goal: x={target_msg.pose.position.x:.2f}, y={target_msg.pose.position.y:.2f}, theta={theta:.2f}"
            )
